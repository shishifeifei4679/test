<template>
  <div class="horizontal-container">
    <div class="scroll-wrapper">
      ...
      >
        <el-tab-pane v-for="(item, index) in topmenus" :key="index" :name="index.toString()">
          <span slot="label" class="labelslot" :data-index="index" :class="{ hideCloseIcon: hideCloseIcon(item) }" v-contextmenu="(el) => contextmenus(el)"> {{item.title }}</span>
        </el-tab-pane>
      </el-tabs>
  ...
  methods: {
    getRouterName(item) {
      if (this.lange == 'en') {
        ...
        const pre = ['mes-', 'wms-'].find((str) => item.path.includes(`/component/${str.slice(0, -1)}/`)) || ''
