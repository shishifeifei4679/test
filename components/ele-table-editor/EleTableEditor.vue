  ...
  <div>
    ...
    <slot>
      <div v-if="isShowAdd" class="ele-table-editor-btn">
        <el-button type="primary" @click="handleAdd">{{ addBtnText }}</el-button>
      </div>
    </slot>

    <!-- element-ui table 组件 -->
    <el-table class="ele-table-editor" :data="value" v-bind="tableAttrs" v-on="tableOn">
      <!-- element-ui table-column组件 -->
      <template v-for="(item, index) of computedColumns">
        <el-table-column v-if="item.render" :label="item.label" v-bind="item">
          <template slot-scope="scope">
            <lb-render :render="item.render" :scope="{ row: value[scope.$index], item }" />
          </template>
        </el-table-column>
        <!-- type 非 index / selection -->
        <el-table-column v-else-if="item.isShowSlot" :key="index" v-bind="item">
          <!-- table-column 作用域插槽 -->
          <template slot-scope="scope">
            <!-- 对外暴露的作用域插槽 -->
            <slot :name="item.prop" :data="value[scope.$index]" :row="scope.row" :disabled="disabled" :index="scope.$index" class="ele-table-editor-content">
              <!-- 判断是否定义了 content 属性 -->
              <template v-if="item.content">
                <template v-for="(contentItem, i) of item.content">
                  <!-- conent 为字符串类型, 直接渲染 span 标签 -->
                  <span :key="i" v-if="typeof contentItem === 'string'">{{ contentItem }}</span>
                  ...
                  <el-tooltip
                    ...
                    :class="{
                      'is-error': isError(scope.$index, contentItem.valueKey, item.prop)
                    }"
                  ...
                  >
                    ...
                    >
                      ...
                      <template v-slot:default v-if="contentItem.options && optionTypes[contentItem.type]">
                        <option-slot
                      ...
                      <!-- 作用域插槽 -->
                      <template v-for="(render, key) of contentItem.scopedSlots" v-slot:[key]="data">
                        <extend-slot :data="data" :key="key" :render="render" />
                      </template>

                      <!-- 非作用域插槽 -->
                      <template v-for="(render, key) of contentItem.slots" v-slot:[key]>
      ...
      <el-table-column v-if="isShowActionColumn" header-align="center" align="center" fixed="right" width="100" :label="$i18n.t('common.operation')">
        <template slot-scope="scope">
          <slot v-bind="scope" name="btn">
            <el-button v-for="(btn, index) of extraBtns" :key="index" v-bind="btn.attrs" :style="btn.style" @click="btn.click(scope)">{{ btn.text }}</el-button>
            <el-button v-bind="deleteBtnAttr" v-if="isShowDelete" @click="handleDelete(scope.$index)">{{$i18n.t('common.delete')}}</el-button>
          </slot>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  name: 'EleTableEditor',
  mixins: [validate],
  components: { ExtendSlot, OptionSlot, lbRender },
  props: {
    // 表格的属性
    tableAttrs: {
      type: Object,
